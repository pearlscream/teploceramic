(function(window, document, $, undefined) {
    var H = $("html"),
        ...
        isScrollable = function(el) {
            return el && !(el.style.overflow && el.style.overflow ===
                "hidden") && (el.clientWidth && el.scrollWidth > el.clientWidth || el.clientHeight && el.scrollHeight > el.clientHeight)
        },
                ...
                case "html":
                    if (current.selector) content = $("<div>").html(content).find(current.selector);
                        ...
                        current.wrap.bind("onReset", function() {
                            if ($(this).find(content).length) content.hide().replaceAll(content.data(placeholder)).data(placeholder,
                                false)
                        })
                    }
                    break;
    ...
    F.transitions = {
        ...
        zoomIn: function() {
            ...
            } else if (effect === "fade") startPos.opacity = .1;
    ...
    F.helpers.overlay = {
        ...
        create: function(opts) {
            ...
            this.overlay = $('<div class="fancybox-overlay"></div>').appendTo(F.coming ?
                F.coming.parent : opts.parent);
            this.fixed = false;
    ...
    $.fn.fancybox = function(options) {
        ...
        if (!selector ||
            options.live === false) that.unbind("click.fb-start").bind("click.fb-start", run);
